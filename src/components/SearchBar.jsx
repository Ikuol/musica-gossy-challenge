...
const SearchBar = () => {
  ...
  const { searchedSong, getSearchedSong } = useContext(songsContext);

  return (
    <form
      autoComplete="off"
      className="p-2 text-gray-400 focus-within:text-gray-600 mt-[15px] w-[32.2%] rounded-[50px]"
    >
      ...
      <div className="text-white absolute top-[75px] left-[75px] max-h-[300px] overflow-hidden bg-[gray]/30 backdrop-blur-sm">
        {searchedSong &&
          searchedSong.map((song, index) => <SearchedSong song={song} key={index} />)}
      </div>
    </form>
  );
};
