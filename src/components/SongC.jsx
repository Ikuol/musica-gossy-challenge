...
const SongC = ({ music }) => {
  ...
  return (
    <div
      onClick={() => getSongToPlay(music.id)}
    ...
    >
      ...
      <div>
        <p className="text-white text-[14px] truncate w-[150px]">{music && music.title}</p>
      </div>
      <div>
        <p className="text-white text-[12px] truncate w-[100px]">{music && music.artist}</p>
      </div>
    </div>
  );
};
