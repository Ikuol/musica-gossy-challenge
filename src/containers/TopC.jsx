...
const TopC = ({imgUrl,title,artist,duration, index}) => {
  return (
    <div className='flex flex-row w-[550px] h-[110px] rounded-[20px] bg-[#1A1E1F] items-center justify-between mr-[90px]'>
            <div className='flex flex-row gap-[15px] text-white items-center ml-4'>
                <div>
                     <img src={ imgUrl } alt="error" className='w-[63px] h-[63px]' />
                </div>
                <div>
                    <NavLink to={`/Album/${index+1}`}><p className='cursor-pointer'>{ title }</p></NavLink>   
